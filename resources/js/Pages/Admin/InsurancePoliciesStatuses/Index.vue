        ...
        <div class="shadow-md sm:rounded-lg relative">
            ...
            <div class="overflow-x-auto">
                <table class="w-full text-sm text-left text-gray-500 dark:text-gray-400">
                    ...
                    <tbody>
                        <tr v-for="(insurance_policies_status, index) in insurance_policies_statuses"
                            class="bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                            <th scope="row"
                                class="flex items-center px-6 py-4 text-gray-900 whitespace-nowrap dark:text-white">
                                <i class="fa-solid fa-tag text-3xl text-indigo-600"></i>
                                <div class="pl-3">
                                    <div class="text-base font-semibold">{{ insurance_policies_status.name }}</div>
                                </div>
                            </th>
                            <td class="px-6 py-4">
                                <div class="flex items-center" v-if="!insurance_policies_status.deleted_at">
                            ...
                            <td class="px-6 py-4">
                                <i @click="showModal('editInsuranceCompany' + insurance_policies_status.id)"
                                    class="fa-solid fa-square-pen text-3xl text-yellow-500 cursor-pointer ml-2"></i>
                                <Edit :id="'editInsuranceCompany' + insurance_policies_status.id" :insurance_policies_status="insurance_policies_status" />
                                <i @click="deleteInsuranceCompany(insurance_policies_status.id)"
...
export default {
    ...
    props: ['insurance_policies_statuses', 'breadcrumbs'],
    data() {
        return {
    ...
    methods: {
        ...
        async deleteInsuranceCompany(id) {
            try {
                const response = await axios.delete('/admin/insurance_policies_statuses/' + id);
